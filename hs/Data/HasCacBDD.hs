...
mgrPtr :: XBddManager
mgrPtr = unsafePerformIO (xBddManager_new 100) -- We fix a maximum of a 100 variables
{-# NOINLINE mgrPtr #-}

top :: Bdd
top = unsafePerformIO (xBddManager_BddOne (unsafePerformIO (bdd_new 8)) mgrPtr)
{-# NOINLINE top #-}

bot :: Bdd
bot = unsafePerformIO (xBddManager_BddZero (unsafePerformIO (bdd_new 8)) mgrPtr)
{-# NOINLINE bot #-}

var :: Int -> Bdd
var n = unsafePerformIO (xBddManager_BddVar (unsafePerformIO (bdd_new 8)) mgrPtr (fromIntegral n))
{-# INLINE var #-}

instance Eq Bdd where
  b1 == b2 = unsafePerformIO (bdd_Operator_Equal b1 b2)
