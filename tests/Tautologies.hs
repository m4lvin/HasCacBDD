...
tests  =
  ...
  ,("firstVarOf",     quickCheckResult (\b -> if b `elem` [bot,top] then isNothing (firstVarOf b) else Just (head (allVarsOfSorted b)) == firstVarOf b))
  ,("maxVarOf",       quickCheckResult (\b -> if b `elem` [bot,top] then isNothing (maxVarOf b) else Just (last (allVarsOfSorted b)) == maxVarOf   b))
  ...
  ,("conSetCommute",  quickCheckResult (\a as -> conSet (a:as) == con (conSet as) a))
  ,("disSetCommute",  quickCheckResult (\a as -> disSet (a:as) == dis (disSet as) a))
  ,("xorSetNeg3",     quickCheckResult (\a b c -> xorSet [a,b,c] == xorSet [neg a, b, neg c]))
  ,("xorSetCommute3", quickCheckResult (\a b c -> xorSet [a,b,c] == xor (xorSet [a,b]) c))
  ,("xorSetCommute4", quickCheckResult (\a b c d -> xorSet [a,b,c,d] == xor (xorSet [b,c,a]) d))
  ...
  ,("subsOf",         quickCheckResult (\b -> all (`elem` subsOf b) (subsOf $ thenOf b)))
